...
function Catagroy() {
  ...
  const handleSubmit = (e) => {
    ...
    const Details = {
      shoename : productname,
      shoeprize : productprize
    }
    console.log(Details);

    //axios

    axios.post("http://localhost:8000/shoe", Details).then((data) => {
      console.log(data);
      if (data.data.error){
                    ...
                    <div className='col s3'>
                      <div className='card'>
                        <div className='card-content'>
                          <p>Shoe name  :&nbsp;&nbsp;&nbsp;{datas.shoename}</p>
                          
                          <p>Shoe prize :&nbsp;&nbsp;&nbsp;{datas.shoeprize}</p>
                        </div>
                      </div>
                      </div>
                    </>
